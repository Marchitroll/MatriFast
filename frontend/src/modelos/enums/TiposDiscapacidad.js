/**
 * Este módulo obtiene y exporta la lista de tipos de discapacidad permitidos desde la base de datos Supabase.
...
// Variable para almacenar la lista de tipos de discapacidad permitidos
let listaDeTiposDiscapacidad = null;

// Consulta a la base de datos para obtener los tipos de discapacidad
if (!listaDeTiposDiscapacidad) {
  const { data, error } = await supabase
    .from('TipoDiscapacidad')
    .select('valor');
  if (error) throw new Error(error.message);
  // Se crea un array inmutable solo con los valores no vacíos y sin espacios
  listaDeTiposDiscapacidad = Object.freeze(
    (data || []).map(r => r.valor.trim()).filter(Boolean)
  );
}

// Exporta la lista de tipos de discapacidad para ser usada en otros módulos
export default listaDeTiposDiscapacidad;

 ...
 */
export const crearUbicacion = (formData) => {
  try {
 ...
 */
export const crearPerfilLinguistico = (formData) => {
  try {
 ...
 */
export const crearDocente = (datosUsuario, datosDocente, documento) => {
  try {
 ...
 */
export const crearRepresentanteLegal = (datosUsuario, datosRL, documento, ubicacion, perfilLinguistico) => {
  try {
    const { email, role } = datosUsuario;
    const { 
      nombres, 
 ...
 */
export const validarCamposRequeridos = (role, formData) => {
  let requiredFields = [];
 ...
 */
export const registrarUsuario = async (datosUsuario, datosEspecificos) => {
  const { role } = datosUsuario;
  let resultado = { success: false, error: null, data: null };
  
  if (role === 'DOCENTE') {
    // Crear documento
    const { documento, error: docError } = crearDocumento(datosEspecificos);
    if (docError) {
    ...
    // Crear docente
    const { docente, error: docenteError } = crearDocente(datosUsuario, datosEspecificos, documento);
    if (docenteError) {
    ...
    // Crear ubicación
    const { ubicacion, error: ubicacionError } = crearUbicacion(datosEspecificos);
    if (ubicacionError) {
    ...
    const { perfilLinguistico, error: lenguasError } = crearPerfilLinguistico(datosEspecificos);
    if (lenguasError) {
    ...
    const { representanteLegal, error: rlError } = crearRepresentanteLegal(
      datosUsuario, 
      datosEspecificos, 
      documento, 
      ubicacion, 

...
class UsuarioCreator extends IUsuarioCreator {
   ...
   * @param {EstudianteFactory} EstudianteFactory - Fabrica de Estudiantes
   */
  ...
  async #crearObjetoEstudiante(datosUsuario, datosEstudiante) {
    //crear el documento del estudiante (se recibe del frontEnd Formulario.jsx)
    const { documento, error: docError } = this.#crearDocumento(datosEstudiante);
    if (docError)
      return { success: false, error: `Error al crear documento: ${docError}` };
